...
const FilmRoom = component$(() => {
  ...
  const applySettings = $(
    (
      filter: "showClips" | "myMentionsOnly" | "selectAuthor" | "selectKeyword",
      value: string = "",
    ) => {
        ...
        <>
          ...
          <div class={styles["mock-player"]}>
            {clipStarted.value ? (
              <div
                id={styles["active"]}
                class={styles["clip-btn"]}
                onClick$={() => (createNoteOpen.value = true)}
              >
                STOP
              </div>
            ) : (
              <div
                id={styles["inactive"]}
                class={styles["clip-btn"]}
                onClick$={() => (clipStarted.value = true)}
              >
                START
              </div>
            )}
          </div>
          {clipNoteOpen.value && <ClipNote close={closeClipNote} />}
